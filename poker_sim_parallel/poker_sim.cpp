...
// used for previous debuggin , checking condition of deck
...
int main()
{
  ...
  */ // moved to function rebuild_deck()
  ...
  #pragma omp parallel for reduction( +: win_rate)
  for(int i = 0; i < n; i++)
  {
    Poker_sim simulate_poker; // Poker simulator object
    simulate_poker.rebuild_deck(); // clean deck up
    simulate_poker.adjust_deck(entries); // remove hole cards from deck
    simulate_poker.shuffle_desk(); // shuffle deck
    simulate_poker.deal_flop();  // deal flop
    bool result = simulate_poker.poker_evaluate(); 
    if(result) // get result from game and adjust win_rate
    {
      win_rate++;
    }
